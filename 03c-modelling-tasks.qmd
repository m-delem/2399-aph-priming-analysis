...
```{r}
# ─── Combining both tables to add tasks as factors ────────────────────────────
df <-
  bind_rows(
    df_explicit_rt |> 
      mutate(task = as.factor("explicit")),
    df_implicit_rt |> 
      mutate(task = as.factor("implicit"))
  )
```
...
formula_4 <- rt ~ (aphantasia + congruence + color + task)^3 + (congruence + color|subjectid)
# intercept and slope on congruence, color, and task by-participant
formula_5 <- rt ~ (aphantasia + congruence + color + task)^3 + (congruence + color + task|subjectid)
...
model_all_workflows_fitted <- 
  tribble(    ~recipe,           ~model,       ~formula,
    model_recipe_glmm, model_specs_glmm, model_formulas
  ) |> 
