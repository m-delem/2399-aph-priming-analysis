...
glmm_bayes <- 
  ...
  set_engine(
    "stan_glmer",
    family  = Gamma(link = "identity"),
    # prior = student_t(df = 10, location = -1.07, scale = 1.15, autoscale = TRUE),
    # prior_intercept = normal(location = 0.5, scale = 0.1),
    iter = 1000,
    cores   = n_cores,
    chains  = n_cores,
    verbose = TRUE,
...
model_glmm_ex <-
  workflow() |> 
  add_recipe(model_recipe_glmm_ex) |> 
  add_model(glmm_inverse_log, formula = formula_2) |> 
  fit(df_explicit_rt) |> 
  extract_fit_engine()
```
