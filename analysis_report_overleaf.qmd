...
#### Explicit effect - distribution

```{r explicit_empirical_vs_modelbased_distrib}
#| label: fig-explicit_empirical_vs_modelbased_distrib
#| fig-cap: "Distributions of the manually calculated congruence effect (red distribution) against the model-based one (blue distribution) in the explicit task."
#| fig-width: 8
#| fig-height: 4
df_congruence_effect |> 
  ggplot() +
...
#### Implicit effect - distribution

```{r implicit_empirical_vs_modelbased_distrib}
#| label: fig-implicit_empirical_vs_modelbased_distrib
#| fig-cap: "Distributions of the manually calculated congruence effect (red distribution) against the model-based one (blue distribution) in the implicit task."
#| fig-width: 8
#| fig-height: 4
...
df_congruence <- 
  df_congruence_effect |> 
  select(subjectid:spatial_imagery, 6, 8) |> 
  rename(
    "congruence_effect_explicit" = congruence_eff_ex_mb,
    "congruence_effect_implicit" = congruence_eff_im_mb
  )
```
...
The parameters of this model for each task are shown in @tbl-model_congruence_effect-1 and @tbl-model_congruence_effect-2. The estimates are negligible and far from significance.
...
# ═══ Modelling the congruence effect ══════════════════════════════════════════

mod_cng_effect_explicit <-
  linear_reg() |>  
  set_engine("lm") |> 
  set_mode("regression") |> 
  fit(congruence_effect_explicit ~ visual_imagery * spatial_imagery,data = df_congruence) |> 
  extract_fit_engine()
...
mod_cng_effect_implicit <-
  ...
  set_engine("lm") |> 
  set_mode("regression") |> 
  fit(congruence_effect_implicit ~ visual_imagery * spatial_imagery,data = df_congruence) |> 
  extract_fit_engine()
...
preds_implicit_congruence_effect <- 
...
# ═══ 3D Variable space - Explicit task ════════════════════════════════════════
...
df_preds_congruence_effect %>% 
  ...
  add_trace(
    ...
    colorscale = "Viridis",
    colorbar = list(title = list(text = "Predicted congruence effect")),
...
# ═══ 3D Variable space - Implicit task ════════════════════════════════════════
...
Although the effects of the mental imagery variables are statistically anecdotal, this method of visualization shows that the model's planes are not flat, reflecting the interaction dynamics between visual and spatial imagery, and allows us to intuitively understand this interaction. These model planes seem to suggest that the higher Congruence effects in high visual imagers are not an even phenomenon, but that **the significant effect we observed in the [inferential analyses](#sec-inferential) might be driven by a sub-groups of non-aphantasics with varying spatial imageries**.
...
#| fig-cap: "Elbow and BIC methods to determine the optimal cluster number."
...
df_clustering <-
  df_congruence |>
  bind_cols(clustering_2) |> 
  bind_cols(clustering_3) |> 
  rename(
    "clusters_2" = "...7",
    "clusters_3" = "...8"
    ) |> 
...
```{r 3d_3_clusters_explicit_plotly}
#| label: fig-3d_3_clusters_explicit
#| fig-cap: "3D mapping of the two clusters along the original variables in the explicit task."
...
```{r 3d_3_clusters_implicit_plotly}
#| label: fig-3d_3_clusters_implicit
#| fig-cap: "3D mapping of the two clusters along the original variables in the implicit task."
...
```{r 3_clusters_repartition_plotly}
#| label: fig-3_clusters_repartition
#| fig-cap: "Repartition of the original groups in the two clusters."

(df_clustering |> 
  mutate(Cluster = clusters_3, .keep = "unused") |> 
  ggplot(aes(x = Cluster, fill = aphantasia)) +
  geom_bar(
    alpha = .5, 
    position = position_dodge(), 
...
To have an intuitive visualization of the mean differences, we will plot them in radar charts in @fig-2_clusters_radar_chart and @fig-3_clusters_radar_chart.

:::{.panel-tabset .column-page-inset-right}

#### 2-Cluster solution

```{r 2_clusters_radar_charts}
#| label: fig-2_clusters_radar_chart
#| fig-cap: "Means of the four clusters in the four variables of interest. The axis have been rescaled to the minimum and maximum values of each variable, to 'zoom' on the differences and compensate for the small effect sizes. Feel free to **click on the legend to see specific clusters**."

(df_clustering %>% 
  ungroup() |> 
  select(clusters_2, visual_imagery:congruence_effect_implicit) %>% 
  mutate(Cluster = as.character(clusters_2), .keep = "unused") %>% 
...
```{r 3_clusters_radar_charts}
#| label: fig-3_clusters_radar_chart
#| fig-cap: "Means of the four clusters in the four variables of interest. The axis have been rescaled to the minimum and maximum values of each variable, to 'zoom' on the differences and compensate for the small effect sizes. Feel free to **click on the legend to see specific clusters**."

(df_clustering %>% 
  ungroup() |> 
  select(clusters_3, visual_imagery:congruence_effect_implicit) %>% 
  mutate(Cluster = as.character(clusters_3), .keep = "unused") %>% 
  mutate(across(visual_imagery:congruence_effect_implicit, normalize)) %>% 
  group_by(Cluster) %>% 
  summarise(across(everything(), mean)) %>% 
  ggradar(
...
We can see that the clusterings highlight various visual/spatial imagery profiles, but do not help in understanding congruence effect differences between the participants. As such, I did not include this analysis in the manuscript for now.
