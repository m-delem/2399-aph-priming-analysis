...
- `data_per_subject` is a table containing one row per subject with all the associated information, raw or computed:
    ...
    - visual_/spatial_imagery are the two PCA components calculated in [the analysis ot questionnaire data](#sec-pca).
    - congruence_effect_explicit/_implicit are the "*Congruece effects per subject*", i.e. the slope by-subject on congruence extracted from the multilevel (mixed) models of the [inferential analysis](#sec-inferential).
    ...
    - explicit/implicit_mean/mad/min/max are the summary statistics of the two main tasks.
...
shelf(
  ...
  ricardo-bion/ggradar,        # gg radar plots
...
The rest of the report is describing and justifying the choices, transformations and assumptions, and visualizing the analyses.
:::
