...
shelf(
  ...
  ggpubr,         # ggplot tools
  ggsci,          # ggplot palettes
  wesanderson,    # ggplot palettes
...
- Bin the participants in two categories based on the VVIQ (cut-off at 32, Aphantasics/Phantasics).

- Use the visual imagery as a continuous variable.
...
We can see that the distributions are skewed yet very similar across groups: as such, we are going to **use accuracy only to remove missed trials in the analysis of the RTs**. Nevertheless, the main noticeable aspect of these plots are the obvious outliers in each of the tasks. This information is crucial as it could reflect a wrong understanding of the task, or random responses: in any case, participants that have high error rates would require a deletion of up to 47% of their trials, which could become a bias in the modelling of the overall data. As there appear to be a cut-off of the main distribution around 16% of errors in all tasks, this value will be chosen as threshold. This amounts to:

- One participant with 24% error rate in the association task

- Five participants with error rates ranging between 44 and 47% in the explicit task

- Four participants with respectively 97, 38, 33 and 22% error rates in the implicit task (at best 7% above everyone else).

The error rates of these ten participants are shown in @tbl-outliers.

```{r outliers}
#| label: tbl-outliers
#| tbl-cap: "Error rates across tasks for the nine outliers in terms of accuracy."

# "jlejeune", "bdispaux", "eleveque", "aleclaire", "aknezevic", "lbrunie", "agayou", "bluciani", "ldossantos", "dchimenton"
df_tasks <- 
  list(df_asso, df_explicit, df_implicit) %>% 
  imap(
    ~.x %>% 
      ...
      mutate(across(subjectid, ~case_when(
        .x == "jlejeune" ~ "Outlier J",
        .x == "bdispaux" ~ "Outlier BD",
        .x == "eleveque" ~ "Outlier E",
        .x == "aleclaire" ~ "Outlier AL",
        .x == "aknezevic" ~ "Outlier AK",
        .x == "lbrunie" ~ "Outlier LB",
        .x == "agayou" ~ "Outlier AG",
        .x == "bluciani" ~ "Outlier BL",
        .x == "ldossantos" ~ "Outlier LD",
        .x == "dchimenton" ~ "Outlier D",
        TRUE ~ .x
      ))) %>% 
...
df_tasks[[1]] <- df_tasks[[1]] %>% rename("Association errors (%)" = prop)
df_tasks[[2]] <- df_tasks[[2]] %>% rename("Explicit errors (%)" = prop) %>% select(2)
df_tasks[[3]] <- df_tasks[[3]] %>% rename("Implicit errors (%)" = prop) %>% select(2)

bind_cols(df_tasks) %>% rename("Participant" = subjectid)%>% display
```

It appears that their high error rate in some tasks do not affect their performance in others.
