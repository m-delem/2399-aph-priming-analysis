...
As a preprocessing, response times below 300ms and above 3000ms will be removed as they could represent false alarms, along with error trials. This amounts to 5.3% of trials in both tasks. As specified earlier, the data for each task from outliers that have an error rate above to 16% will be removed as well (9 participants in the explicit task and 4 in the implicit task). There could also be outliers with overall abnormally fast or slow RTs. This analysis will be conducted on mean RTs, the distribution of which is displayed in @fig-rt_means, along with their statistics in @tbl-rt_means. 
...
The mixed models will be fitted using `tidymodels` workflows. RT data will be standardized before modelling, therefore the standardized coefficients reported will scale on *standard deviations of the RT variable*. Let's try displaying some parameters in @tbl-model_test_table.
...
# df_explicit_rt <- df_explicit_rt %>% standardize(rt_explicit)
# df_implicit_rt <- df_implicit_rt %>% standardize(rt_implicit)
...
rt_recipe_explicit <- 
  ...
  step_normalize(age, rt_explicit)
...
rt_recipe_implicit <- 
  ...
  step_normalize(age, rt_implicit) 

# --- workflows ---
rt_workflow_explicit <- workflow() %>% add_recipe(rt_recipe_explicit)
rt_workflow_implicit <- workflow() %>% add_recipe(rt_recipe_implicit)
...
```{r specifying_group_models}
#| echo: false

# --- Specifying group models ---

# ----------------------------------------------------------- Explicit task ----

# model 1: intercepts only
explicit_g_intercept <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ (1|subjectid)
    )

# model 2: main fixed effects
explicit_g_fixed <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
...
# model 3: group congruence interaction
explicit_g_2_way_gxcn <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ aphantasia * congruence + color + (1|subjectid)
    )

# model 4: congruence color interaction
explicit_g_2_way_cnxcl <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ aphantasia + congruence * color + (1|subjectid)
    )

# model 5: group color interaction
explicit_g_2_way_gxcl <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ aphantasia + congruence + color + aphantasia:color + (1|subjectid)
    )

# model 6: three way interactions
explicit_g_3_way <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ aphantasia * congruence * color + (1|subjectid)
...
# model 7: reduced model without color
explicit_g_2_way_nocolor <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ aphantasia * congruence + (1|subjectid)
    )
  
# model 8: congruence slope by subject
explicit_g_2_way_nocolor_slope <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ aphantasia * congruence + (congruence|subjectid)
    )
...
# model 1: intercepts only
rt_im_modg1 <-
  rt_workflow_implicit %>% add_model(
    rt_model_lmer,
...
```{r specifying_continuous_models}
#| echo: false

# --- Specifying continuous models ---
...
rt_ex_modcvi2 <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ visual_imagery + congruence + color + (1|subjectid)
    )
...
rt_ex_modcvi5 <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ visual_imagery + congruence + color + aphantasia:color + (1|subjectid)
    )
...
rt_ex_modcvi7 <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ visual_imagery * congruence + (1|subjectid)
    )

# model 8: slope by visual imagery
rt_ex_modcvi8 <-
  rt_workflow_explicit %>% add_model(
    rt_model_lmer,
    formula = rt_explicit ~ visual_imagery * congruence + (1 + visual_imagery|subjectid)
    )
...
rt_im_modcvi2 <-
  rt_workflow_implicit %>% add_model(
    rt_model_lmer,
    formula = rt_implicit ~ visual_imagery + congruence + color + (1|subjectid)
    )
...
rt_im_modcvi5 <-
  rt_workflow_implicit %>% add_model(
    rt_model_lmer,
    formula = rt_implicit ~ visual_imagery + congruence + color + aphantasia:color + (1|subjectid)
    )
...
fit_rt_ex_gmods <-
  list(
    explicit_g_intercept = explicit_g_intercept, 
    explicit_g_fixed = explicit_g_fixed, 
    explicit_g_2_way_gxcn = explicit_g_2_way_gxcn, 
    explicit_g_2_way_cnxcl = explicit_g_2_way_cnxcl, 
    explicit_g_2_way_gxcl = explicit_g_2_way_gxcl, 
    explicit_g_3_way = explicit_g_3_way,
    explicit_g_2_way_nocolor =  explicit_g_2_way_nocolor,
    explicit_g_2_way_nocolor_slope = explicit_g_2_way_nocolor_slope
    ) %>% 
...
fit_rt_im_gmods <-
  list(
    implicit_g_intercept = rt_im_modg1, 
    implicit_g_fixed = rt_im_modg2, 
    implicit_g_2_way_gxcn = rt_im_modg3, 
    implicit_g_2_way_cnxcl = rt_im_modg4, 
    implicit_g_2_way_gxcl = rt_im_modg5, 
    implicit_g_3_way = rt_im_modg6
    ) %>% 
...
fit_rt_ex_cvimods <-
  list(
    explicit_cvi_intercept = rt_ex_modcvi1, 
    explicit_cvi_fixed = rt_ex_modcvi2, 
    explicit_cvi_2_way_gxcn = rt_ex_modcvi3, 
    explicit_cvi_2_way_cnxcl = rt_ex_modcvi4, 
    explicit_cvi_2_way_gxcl = rt_ex_modcvi5, 
    explicit_cvi_3_way = rt_ex_modcvi6,
    explicit_cvi_2way_nocolor = rt_ex_modcvi7,
    explicit_cvi_2_way_nocolor_slope = rt_ex_modcvi8
   ) %>% 
  imap(~ .x %>% fit(data = df_explicit_rt) %>% extract_fit_engine())
...
fit_rt_im_cvimods <-
  list(
    implicit_cvi_intercept = rt_im_modcvi1, 
    implicit_cvi_fixed = rt_im_modcvi2, 
    implicit_cvi_2_way_gxcn = rt_im_modcvi3, 
    implicit_cvi_2_way_cnxcl = rt_im_modcvi4, 
    implicit_cvi_2_way_gxcl = rt_im_modcvi5, 
    implicit_cvi_3_way = rt_im_modcvi6
    ) %>% 
  imap(~ .x %>% fit(data = df_implicit_rt) %>% extract_fit_engine())
...
fit_rt_all_mods <-
  list(
    explicit_grouped = fit_rt_ex_gmods,
    implicit_grouped = fit_rt_im_gmods,
    explicit_cvi = fit_rt_ex_cvimods,
    implicit_cvi = fit_rt_im_cvimods
    )

# --- Performance of the models ------------------------------------------------

fit_rt_all_performances <- 
  fit_rt_all_mods %>% 
  imap(
    ~ .x %>% 
      compare_performance %>% 
...
# --- Reporting parameter estimations ------------------------------------------

fit_rt_all_parameters <-
  fit_rt_all_mods %>% 
  imap(
    ~ .x %>%
      imap(
...
```{r model_test_table}
#| echo: false
#| label: tbl-model_test_table
#| tbl-cap: "Comparative performances of the fitted models."

fit_rt_all_performances$explicit_grouped %>% display

fit_rt_all_mods$explicit_cvi$explicit_cvi_2_way_gxcn %>% report_model

fit_rt_all_mods$explicit_cvi$explicit_cvi_2_way_nocolor_slope %>% 
  estimate_relation(
    include_random = TRUE
    ) %>%
  plot +
