...
I struggled to understand the variables used for clustering in the actual state of the manuscript. The algorithms need continuous variables, and I only found 3: *Visual imagery* (VVIQ), *Congruence effect* (a transformed RT difference variable) in the *Explicit* and *Implicit* tasks. I would add a fourth one, the Spatial component of the PCA, in the exploratory process. However there is a mention of adding *Color* as a clustering variable in the process: how was this possible? The question comes from the fact that Color is a sub-structure of the task, therefore adding Color would mean adding *four* variables instead of two (Colored or Uncolored in Explicit or Implicit), along with 
:::
