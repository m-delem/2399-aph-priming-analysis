...
rt_ex_modg1 <-
  ...
  add_model(
    spec = rt_model_lmer,
    formula = rt_explicit ~ 
      (1|subjectid) #+ (1|orientation)
    )

# model 2: principal main effects
rt_ex_modg2 <-
  rt_ex_modg1 %>%
  update_model(
    spec = rt_model_lmer,
...
# model 3: spatial and age effects
rt_ex_modg3 <-
  rt_ex_modg1 %>%
  update_model(
    spec = rt_model_lmer,
...
# model 4: two and three way interactions
rt_ex_modg4 <-
  rt_ex_modg1 %>%
  update_model(
    spec = rt_model_lmer,
...
rt_ex_modcvi2 <-
  ...
  update_model(
    spec = rt_model_lmer,
    formula = rt_explicit ~ 
      visual_imagery + congruence + color + 
      (1|subjectid) #+ (1|orientation)
    )
...
fit_rt_ex_gmods <-
  list(
    ...
    explicit_g_2_way = rt_ex_modg3, 
    explicit_g_3_way = rt_ex_modg4
    ) %>% 
  imap(~ fit(.x, df_explicit_rt) %>% extract_fit_engine())

fit_rt_im_gmods <-
  list(
    implicit_g_intercept = rt_im_modg1, 
    implicit_g_fixed = rt_im_modg2, 
    implicit_g_2_way = rt_im_modg3, 
    implicit_g_3_way = rt_im_modg4
    ) %>% 
  imap(~ fit(.x, df_implicit_rt) %>% extract_fit_engine())

fit_rt_ex_cvimods <-
  list(
   explicit_cvi_intercept = rt_ex_modcvi1, 
   explicit_cvi_fixed = rt_ex_modcvi2, 
   explicit_cvi_2_way = rt_ex_modcvi3, 
   explicit_cvi_3_way = rt_ex_modcvi4
   ) %>% 
  imap(~ fit(.x, df_explicit_rt) %>% extract_fit_engine())
...
fit_rt_im_cvimods <-
  list(
    ...
    implicit_cvi_2_way = rt_im_modcvi3, 
    implicit_cvi_3_way = rt_im_modcvi4
    ) %>% 
  imap(~ fit(.x, df_implicit_rt) %>% extract_fit_engine())
```
...
# fit_rt_ex_cvimods[[1]] %>% report_text %>% summary
# fit_rt_ex_cvimods[[1]] %>% report_table %>% summary
# fit_rt_ex_cvimods[[1]] %>% report_parameters %>% summary
# fit_rt_ex_cvimods[[1]] %>% check_model()  # assumptions check
...
fit_rt_all_mods %>% 
  imap(
    ~ compare_performance(.x)
    # ~ bayesfactor_models(.x)
      # report %>% 
      # summary
    )


fit_rt_ex_gmods %>% compare_performance          # bad
fit_rt_ex_gmods %>% bayesfactor_models %>% report %>% summary # better
...
rt_ex_modg2 <-
  ...
  update_model(
    spec = rt_model_lmer,
    formula = rt_explicit ~ aaphantasia * congruence * color * spatial_imagery + (1|subjectid) + (1|orientation))
