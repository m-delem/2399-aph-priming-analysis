...
-   `data_per_subject` is a table containing one row per subject with all the associated information, raw or computed:
    ...
    -   visual\_/spatial_imagery are the two PCA components calculated in [the analysis of questionnaire data](#sec-pca).
...
Although the effects of the mental imagery variables are statistically anecdotal, this method of visualization shows that the model's planes are not flat, reflecting the interaction dynamics between visual and spatial imagery, and allows us to

intuitively understand this interaction. Some interesting observations (you can rotate @fig-3d_congruence_effect_explicit and [-@fig-3d_congruence_effect_implicit] to visualize the effects described here):
...
all_models_fitted[["explicit_mods"]][["mod_explicit_4"]] |> check_model(check = "pp_check", dot_alpha = .2)
all_models_fitted[["explicit_mods"]][["mod_explicit_4"]] |> check_model(check = "homogeneity", dot_alpha = .2)
```
...
# --- preprocessing recipe for explicit data, no Box-Cox ---
recipe_explicit_glmm <- 
  df_explicit_rt %>%
  recipe() %>%
  update_role(
    rt_explicit, 
...
# --- preprocessing recipe for implicit data, no Box-Cox  ---
recipe_implicit_glmm <-
  df_implicit_rt %>%
  recipe() %>%
  update_role(
    rt_implicit, 
...
# --- workflows ---
workflow_explicit_glmm <- workflow() %>% add_recipe(recipe_explicit_glmm)
workflow_implicit_glmm <- workflow() %>% add_recipe(recipe_implicit_glmm)
...
# maximal glmm
glmm_explicit <-
  workflow_explicit_glmm |> 
  add_model(
    mod_glmer,
    formula = rt_explicit ~ aphantasia * congruence * color + 
      (congruence|subjectid)
  )
