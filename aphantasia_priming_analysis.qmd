...
Once again, while fairly obvious in visual data exploration, these interaction dynamics between variables have shown to be weak and statistically insignificant. This failure to explain continuous data is reminiscent of another earlier finding, that *Visual imagery is not correlated with Congruence effect*, although we clearly showed in @sec-inferential that the global effect of $Congruence$ varied with the $Group$ (i.e. visual imagery group) in both tasks. This reflects the high variability in the sample both in Congruence effect and mental imagery, variance that is only partly explained by the division in two groups based on visual imagery (although this division already yields - very small - significant interaction effects).
