...
# The mixed models will be fitted using `tidymodels` workflows and methods.
...
# A forward procedure is going to be used to assess the quality of the models, as there is a huge potential amount of predictors.
...
# explicit task with groups
fit_rt_ex_gmods <-
  list(
    explicit_g_intercept = rt_ex_modg1, 
    explicit_g_fixed = rt_ex_modg2, 
    explicit_g_2_way_gxcn = rt_ex_modg3, 
    explicit_g_2_way_cnxcl = rt_ex_modg4, 
    explicit_g_2_way_gxcl = rt_ex_modg5, 
    explicit_g_3_way = rt_ex_modg6
    ) %>% 
  imap(~ .x %>% fit(data = df_explicit_rt) %>% extract_fit_engine())

# implicit task with groups
fit_rt_im_gmods <-
  list(
...
# explicit task with continuous imagery variable
fit_rt_ex_cvimods <-
  list(
...
# implicit task with continuous imagery variable
fit_rt_im_cvimods <-
  list(
