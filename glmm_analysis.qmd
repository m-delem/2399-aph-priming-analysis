...
# ─── Specifying the types and distributions for the models ────────────────────
...
# LMM (i.e. GLMM with a Gaussian distribution)
lmm_gauss <-
  linear_reg() |> 
  set_engine("lmer")
...
# LMM
model_spec_lmm <- list(lmm_gauss = lmm_model)
...
model_all_workflows_fitted <- 
  tribble( ~recipe,     ~ task,          ~models,      ~formulas,
    glmm_recipe_ex, "explicit", model_specs_glmm, model_formulas,
    glmm_recipe_im, "implicit", model_specs_glmm, model_formulas,
     lmm_recipe_ex, "explicit",   model_spec_lmm, model_formulas,
     lmm_recipe_im, "implicit",   model_spec_lmm, model_formulas,
  ) |> 
  # combining recipes with models
  unnest_longer(models) |> 
  # combining recipes and models with formulas
  unnest_longer(formulas) |>
  rowwise() |>
  mutate(
    # creating workflows
    workflow = list(
      workflow() |>
      add_recipe(recipe) |> 
      add_model(models, formula = formulas)  
      ),
    # fitting all the workflows
    fitted_model = ifelse(
      task == "explicit",
    ...
    parameters = model_parameters(fitted_model),
    # checking convergence of the models
    convergence = check_convergence(engine),
    # model quality indices
    model_perf = model_performance(engine),
    AIC  = model_perf[[1]],
    AICc = model_perf[[2]],
    BIC  = model_perf[[3]]
  ) |>
  ungroup() |> 
  select(-c(recipe, models, formulas, workflow, model_perf))

model_selection <-
  model_all_workflows_fitted |> 
  filter(formulas_id != "formula_0") |> 
  group_by(task, models_id) |> 
...
# glmm intercept
glmm_intercept <- (glmm_ex_optimal |> model_parameters())$Coefficient[1]
...
# lmm intercept
lmm_intercept <- (lmm_ex_optimal |> model_parameters())$Coefficient[1]
...
df_explicit_rt |> 
  ...
  # intercept
  geom_hline(yintercept = glmm_intercept, linetype = 2) +
  labs( 
    x = NULL, 
    y = "Response time (ms)"
    ) +
...
lmm_recipe_ex|> 
  ...
  # intercept
  geom_hline(yintercept = lmm_intercept, linetype = 2) +
  labs( 
    x = NULL, 
    y = "Response time (standardized)"
    ) +
  scale_x_discrete(labels = c("Congruent", "Incongruent")) +
  scale_fill_okabeito(name = "Aphantasia: ", labels = c("Yes", "No")) +
  scale_color_okabeito(guide = NULL)
...
glmm_preds_im <- 
  ...
  estimate_means(at = c("aphantasia", "congruence"))
# glmm intercept
glmm_intercept_im <- (glmm_im_optimal |> model_parameters())$Coefficient[1]
...
# lmm intercept
lmm_intercept_im <- (lmm_im_optimal |> model_parameters())$Coefficient[1]
...
df_implicit_rt |> 
  ...
  # intercept
  geom_hline(yintercept = glmm_intercept_im, linetype = 2) +
  labs( 
    x = NULL, 
    y = "Response time (ms)"
    ) +
...
lmm_recipe_im|> 
  ...
  # intercept
  geom_hline(yintercept = lmm_intercept_im, linetype = 2) +
  labs( 
    x = NULL, 
    y = "Response time (standardized)"
    ) +
