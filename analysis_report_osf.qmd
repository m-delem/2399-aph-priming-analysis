...
The best model for the RTs in the implicit task included a random intercept and slope by participant on the Congruence condition (AIC = 22,109, cAIC = 22,109, null model AIC = 22,116), and had a substantial explanatory power (conditional $R^{2}$ = .35). The parameters of this model are displayed in Table \ref{tbl:model_b_implicit}. The significant parameters in this task are the interaction between VI and Congruence, along with a trend towards an interaction between VI and SI.

The interaction between VI and Congruence shows that a higher VI is associated with slower responses in the incongruence condition ($\beta$  = 0.03, SE = 0.01, 95\% CI [0.01, 0.05], t(8874) = 2.58, p = .01). The trend interaction between VI and SI ($\beta$  = 0.06, SE = 0.03, 95\% CI [0.00, 0.12], t(8874) = 1.83, p = .067) once again shows

($\beta$ = 0.3, SE = 0.01, 95\% CI [0.05, .26], t(8877) = 2.91, p = .004) also indicates that controls responded slower in the incongruent condition, this contrast being the only significant one in this interaction (marginal contrast (controls congruent $RT_{std}$ - incongruent $RT_{std}$) = -0.11, CI = [-0.19; -0.02], SE = 0.03, p = .01). These effects are represented in Figure \ref{fig:model_b_implicit}. Both estimates are close to 0.2, which denotes a small effect size.
...
p_b_mod_im_visi <- 
  ...
  theme(
    ...
    legend.title = element_text(size = txt/2)
    )
```
